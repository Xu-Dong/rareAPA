...
# modified by Xudong Zou, 2021-07-29

# Takes normalized pdui data (PEER corrected by Lei)
...
if len(sys.argv) != 5:
        print "usage: gather_filter_normalized_pdui_peerless.py tissueFile indFile peerSuffix outFile"
        sys.exit()
...
# @zouxd, define a function to format individual id
def format_name(x):
    return("-".join(x.split(".")))
...
for tissue in tissues:
	...
	headerList = expr.readline().strip().split()
        headerList = list(map(format_name,headerList))#format individual name
