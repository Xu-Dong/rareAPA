...
ratio_list <- c()
for(i in 1:1000){
	aOutlier_sub <- sample(aOutlier_genes,N,replace=TRUE)
	m <- length(intersect(aOutlier_sub,aGene_list))
	ratio_list[i] <- m/N
}
Ratio_Mean <- mean(ratio_list)
...
write.table(df_out,file=paste0("./output/bootstrap_z3/",curr_tissue,".bootstrap_ratio.Z3.txt"),quote=F,sep="\t",row.names=F)
