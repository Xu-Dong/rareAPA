...
#file_results=os.getcwd()+"/../output/results_li"
...
file_sentinalSNP="/lustre/home/lhgong/share/ukbb_coloc/sentinal_data"
path_output=file_results+"/../summary_coloc/allTissueProportion_SigSentinalSnp_range1e5_hyp4_0.75.txt"
...
for each_trait in file_trait:
	...
	for each_trait_tissue in trait_folder:
		...
		# K11_HERNIA.sentinal.txt
		file_sentinal=file_sentinalSNP+"/"+each_trait+".sentinal.txt"
		#print(file_sentinal)
	...
	for each_trait_tissue in trait_folder:
		...
		for line in f1:
			...
			if line_split[5] !="NA" and line_split[6] !="NA":
				...
				#if hyp4>0.75:
				#if hyp4>0.75 and hyp4/(hyp3+hyp4)>=0.9:#by HYM
				if hyp4>0.75:#by HYM
