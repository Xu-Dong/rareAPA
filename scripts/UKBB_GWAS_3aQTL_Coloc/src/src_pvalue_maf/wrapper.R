...
library(coloc,lib.loc="/lustre/home/rfding/anaconda3/envs/work/lib/R/library/")
...
for (row in seq(from=1,to=length(traits$filepath))){
  ...
  sys_arg <- paste("/lustre/home/wychen/anaconda3/envs/r4/bin/Rscript make_gene_trait_tables.R ",
                   "--","file ",file, " ",
                   "--","trait ",trait, " ",
...
for (file in merged_table_files){
  ...
  sys_arg <- paste("/lustre/home/wychen/anaconda3/envs/r4/bin/Rscript get_coloc_summaries.R ",
                   "--","file ",file, " ",
                   sep="") #edit by CWY
  # NOTE: wait=TRUE means R will submit a job, wait for it to finish, and then
  # continue. Setting it to FALSE utilizes the parallel computing capabilities of
