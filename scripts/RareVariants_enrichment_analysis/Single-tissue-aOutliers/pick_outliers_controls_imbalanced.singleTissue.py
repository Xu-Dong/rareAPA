...
def make_test_dict(COUNTS, individs):
	'''
	...
	header = header[indices_to_keep[0:]]
...
#		counts = np.array([float(x) for x in counts_line[1:]])
	#	tested = counts >= count_threshold
		keep_idx = [i for i in range(1,len(counts_line)) if counts_line[i]!="NA"]
		test_dict[gene] = header[keep_idx]
	return test_dict
...
#	test_dict = make_test_dict(COUNTS, individs, count_threshold)
	test_dict = make_test_dict(COUNTS, individs)
