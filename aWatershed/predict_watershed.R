...
learn_watershed_model_parameters_from_training_data <- function(training_input_file, number_of_dimensions, model_name, pseudoc, lambda_init, binary_pvalue_threshold, lambda_costs, nfolds, vi_step_size, vi_threshold) {
	...
	## Load in data
	########################
	data_input <- load_watershed_data(training_input_file, number_of_dimensions, 0.05, binary_pvalue_threshold)
...
predict_watershed_posteriors <- function(watershed_object, prediction_input_file, number_dimensions) {
	...
	########################
	prediction_data_input <- load_watershed_data(prediction_input_file, number_of_dimensions, .05, binary_pvalue_threshold)
	# Parse data_input for relevent data
...
arguments <- parse_args(OptionParser(usage = "%prog [options]", description="Watershed command line args",
	option_list=list(
		...
		make_option(c("-p","--dirichlet_prior_parameter"), default=0.1, help="Parameter defining Dirichlet distribution the acts as a prior a Phi (the model parameters defining E|Z"),
		make_option(c("-l","--l2_prior_parameter"), default=0.005, help="Parameter defining L2 (gaussian) distribution the acts as a prior on the parameters of the conditional random Field (the model parameters defining Z|G"),
